  ...
  return (
    <Grid item xs={12} lg={3} style={{ borderRadius: 8, border: '2px solid #2980b9', marginTop: 10 }}>
      <Grid container alignItems="center" justifyContent="space-evenly" style={{ paddingBlock: '10px' }}>
        ...
        <Grid item xs={5}>
          <TextField name="text" label="Enter Text to be split into tags" variant="outlined" style={{ width: '150%' }} value={text} onChange={handleChange} />
        </Grid>
        <Grid item xs={5}>
          <StyledButton onClick={handleClick}>Create Tags</StyledButton>
          <Grid container></Grid>
        </Grid>
        <Grid item xs={11} style={{ height: 60, textAlign: 'center' }}>
          {tags.length ? (
            <Grid container className={classes.chipContainer}>
              {tags.map((item, idx) => (
                <Grid item key={idx} className={classes.chipItem}>
